...
function Messages(props) {
  ...
  const populateMessagePanelList = () => {
    ...
    if (dbuser && dbuser.is_creator) {
      getUserList();
    } // Only get user list if we are a shelter (creator)
  ...
  const getChatMessages = () => {
    ...
    newChatMessages.push({
      date: new Date(),
      message:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed convallis, leo ac gravida hendrerit, metus nunc faucibus ante, eu consectetur lectus neque gravida sapien. ",
    });
    newChatMessages.push({
      date: new Date(),
    ...
    newChatMessages.push({
      date: new Date(),
      message:
        "Sed ipsum nulla, aliquam ac congue et, facilisis et neque. Nunc ac tellus sit amet justo faucibus interdum et sit amet quam.",
    });
    newChatMessages.push({
      date: new Date(),
      message: "Donec at accumsan risus, ac tristique sem.",
    });
    newChatMessages.push({
      date: new Date(),
      message:
        " Phasellus facilisis sagittis mauris, pretium vestibulum nulla porttitor non.",
    });
    newChatMessages.push({
      date: new Date(),
      message:
        "Maecenas vulputate rutrum ipsum quis blandit. Phasellus metus lacus, volutpat vel interdum vel, cursus in mi.",
    });
  ...
  }, [dbuser]); // If user changes, get petlist again
  ...
  return (
    <div className="messages-page-container">
      ...
      <MessageFriendsPanel
        ...
        petlist={petlist}
        userlist={userlist}
        handlePetNameClick={handlePetNameClick}
        ...
        {disableInput ? (
          <form className="chat-input-form" onSubmit={sendMessage}>
            <input
              ...
              disabled
              type="text"
              placeholder="Select pet to send message"
              onChange={(e) => setChatValue(e.target.value)}
            />
            <button className="chat-input-submit-button" disabled type="submit">
              Send
            </button>
          </form>
        ) : (
    ...
    <div className="petnames-display">
      ...
      {dbuser && dbuser.is_creator && (
        <div className="user-list-div">
          <select
            className="user-list-div-select"
            name="profilename"
            id="profilename"
      ...
      {petlist &&
        petlist.map((pet, index) => (
          <div
            key={index}
            onClick={() => handlePetNameClick(pet.external_pet_id)}
          >
